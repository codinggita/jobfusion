...
const Template04 = () => {
  ...
  const [resumeData, setResumeData] = useState({
    ...
    professionalExperience: [
      {
        id: '1',
        company: 'Accelerate Software',
        period: 'February 2017 - Present',
        title: 'Senior Account Manager',
        responsibilities: [
          'Oversee 35+ enterprise customer accounts generating over $20M+ in yearly revenue, build strategic partnerships with Fortune 500 clients, and lead initiatives to drive business development efforts and accelerate account growth.',
          'Lead sales meetings with C-level executives to close new business.',
          'Serve as the point of contact for client accounts and identify value-added services to drive customer success, which has improved account growth by 25% on average.',
          'Lead a team of 10+ Account Executives, provide training and mentorship on sales best practices, and deliver support throughout the sales cycle.',
        ],
      },
      {
        id: '2',
        company: 'Celeste Inc.',
        period: 'June 2013 - February 2017',
        title: 'Sales Account Executive',
        responsibilities: [
          'Managed 20+ client accounts generating $150K to $800K in annual revenue.',
          'Attended high-level sales meetings, educated prospects on technology products, and recommended customer solutions based on client business needs.',
          'Drove business development activities, qualified leads, identified potential prospects, and developed new strategies to enhance sales performance.',
          'Collaborated with cross-functional teams and the Marketing Department to enhance product visibility and drive revenue across emerging markets.',
        ],
      },
    ],
    education: [
      {
        id: '1',
        degree: 'Bachelor of Science (B.S.) in Marketing',
        university: 'University of Florida, Gainesville, FL',
        period: 'September 2011 - June 2013',
      },
      {
        id: '2',
        degree: 'Associate of Arts in Business',
        university: 'City Colleges, Chicago, IL',
        period: 'January 2008 - September 2010',
      },
    ],
    certifications: [
      'Certified Professional Sales Leader (CPSL), NASP, 2017',
      'Salesforce Certification, Salesforce.com, 2015',
      'Certified Sales Development Representative, AA-ISP, 2013',
      'Professional Program Certificate in Retail, Tacoma College, 2016',
    ],
    socialLinks: {
  ...
  const handleDragEnd = (result) => {
    ...
    // Handle experience items reordering
    if (type === 'experience') {
    ...
    // Handle education items reordering
    if (type === 'education') {
  ...
  // Filter sections based on visibility and layout
  ...
  const contentSections = resumeData.sectionOrder.filter(
    (section) =>
      ['professionalExperience', 'education', 'certifications'].includes(section) &&
  ...
  const Section = ({ title, children, collapsible = true }) => {
  ...
  const Notification = ({ show, message }) => (
    <div
      className={`fixed bottom-4 right-4 bg-green-600 text-white px-4 py-2 rounded-md shadow-lg transition-opacity flex items-center gap-2 ${show ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
    >
      <Check size={16} />
      <span>{message}</span>
  ...
  const ResumePreview = () => (
    <div
      id="resume"
      ref={resumeRef}
      className="shadow-lg rounded-lg overflow-hidden flex flex-col bg-white"
      style={{ maxWidth: '100%', fontSize: resumeData.fontSize }}
    >
      {/* Header */}
      <div
        style={{
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-xl mt-1">{resumeData.title}</p>
        </div>
      </div>
              ...
              <div className="space-y-2">
                {resumeData.socialLinks.linkedin && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>{resumeData.socialLinks.linkedin}</span>
                  </div>
                )}
                {resumeData.socialLinks.github && (
                  <div className="flex items-center gap-2">
                    <Github className="h-4 w-4" />
                    <span>{resumeData.socialLinks.github}</span>
                  </div>
                )}
                {resumeData.socialLinks.twitter && (
                  <div className="flex items-center gap-2">
                    <Twitter className="h-4 w-4" />
                    <span>{resumeData.socialLinks.twitter}</span>
                  </div>
                )}
                {resumeData.socialLinks.website && (
                  <div className="flex items-center gap-2">
                    <Globe className="h-4 w-4" />
                    <span>{resumeData.socialLinks.website}</span>
                  </div>
                )}
              </div>
            </div>
                      ...
                      <div className="space-y-3">
                        <Input
                          ...
                          onChange={(e) =>
                            setResumeData({
                              ...resumeData,
                              socialLinks: { ...resumeData.socialLinks, linkedin: e.target.value },
                            })
                          }
                          placeholder="LinkedIn URL"
                          prefix={<Linkedin size={16} className="text-gray-400 mr-2" />}
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setResumeData({
                              ...resumeData,
                              socialLinks: { ...resumeData.socialLinks, github: e.target.value },
                            })
                          }
                          placeholder="GitHub URL"
                          prefix={<Github size={16} className="text-gray-400 mr-2" />}
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setResumeData({
                              ...resumeData,
                              socialLinks: { ...resumeData.socialLinks, twitter: e.target.value },
                            })
                          }
                          placeholder="Twitter URL"
                          prefix={<Twitter size={16} className="text-gray-400 mr-2" />}
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setResumeData({
                              ...resumeData,
                              socialLinks: { ...resumeData.socialLinks, website: e.target.value },
                            })
                          }
                          placeholder="Personal Website URL"
                          prefix={<Globe size={16} className="text-gray-400 mr-2" />}
                        />
                      </div>
                    </Section>
                  </Card>
                    ...
                    <Section title="Colors">
                      <div className="grid grid-cols-2 gap-4">
                        <ColorPicker
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, primaryColor: e.target.value })
                          }
                        />
                        <ColorPicker
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, secondaryColor: e.target.value })
                          }
                        />
                        <ColorPicker
                          label="Accent Color"
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, accentColor: e.target.value })
                          }
                        />
                        <ColorPicker
                          label="Header Text"
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, headerTextColor: e.target.value })
                          }
                        />
                        <ColorPicker
                          label="Sidebar Text"
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, sidebarTextColor: e.target.value })
                          }
                        />
                        <ColorPicker
                          label="Main Text"
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, mainTextColor: e.target.value })
                          }
                        />
                        <ColorPicker
                          label="Background"
                          ...
                          onChange={(e) =>
                            setResumeData({ ...resumeData, backgroundColor: e.target.value })
                          }
                        />
                      </div>
                    </Section>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setResumeData({ ...resumeData, fontFamily: e.target.value })
                            }
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setResumeData({ ...resumeData, fontSize: e.target.value })
                            }
                            className="w-full p-2 border rounded"
                          >
                      ...
                      <div className="space-y-3">
                        <Toggle
                          ...
                          onChange={() =>
                            setResumeData({
                              ...resumeData,
                              layout: resumeData.layout === 'sidebar-left' ? 'sidebar-right' : 'sidebar-left',
                            })
                          }
                        />
                      </div>
